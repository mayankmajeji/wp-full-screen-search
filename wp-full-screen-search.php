...
define('WPFSS_PLUGIN_VERSION', '1.1.0');
...
if (! class_exists('WP_Full_Screen_Search')) {
	...
	class WP_Full_Screen_Search
	{
		 ...
		 */
		public function wpfss_dashboard_notices()
		{
			global $pagenow;

			if (! get_option($this->db_welcome_dismissed_key)) {
				if (! ('options-general.php' === $pagenow && isset($_GET['page']) && 'wp-full-screen-search' === $_GET['page'])) {
		 ...
		 */
		public function wpfss_dismiss_dashboard_notices()
		{
		 ...
		 */
		public function wpfss_register_menu()
		{
		 ...
		 * Register Settings
		 */
		public function wpfss_register_settings()
		{
		 ...
		 */
		public function wpfss_settings_page()
		{
			...
			if (isset($_REQUEST['submit'])) {
				...
				if (! isset($_REQUEST[$this->plugin_name . '_nonce'])) {
					...
					// Missing nonce
					$this->error_message = esc_html__('nonce field is missing. Settings NOT saved.', 'wp-full-screen-search');
				} elseif (! wp_verify_nonce($_REQUEST[$this->plugin_name . '_nonce'], $this->plugin_name)) {

					// Invalid nonce
					$this->error_message = esc_html__('Invalid nonce specified. Settings NOT saved.', 'wp-full-screen-search');
				} else {
		...
		public function wpfss_admin_assets()
		{
			...
			wp_enqueue_script('wp-color-picker-alpha', WPFSS_PLUGIN_URI . '/assets/js/wpfss-wp-color-picker-alpha.js', array('wp-color-picker'), WPFSS_PLUGIN_VERSION, true);

			// Enqueue admin styles
			wp_enqueue_style('wpfss-admin-style', WPFSS_PLUGIN_URI . '/assets/css/wpfss-admin-style.css', array(), WPFSS_PLUGIN_VERSION, false);

			wp_enqueue_script('wpfss-admin-script', WPFSS_PLUGIN_URI . '/assets/js/wpfss-admin-script.js', array('wp-color-picker'), WPFSS_PLUGIN_VERSION, true);
		}
		...
		{
			// Enqueue admin styles
			wp_enqueue_style('wpfss-style', WPFSS_PLUGIN_URI . '/assets/css/wpfss-style.css', array(), (WPFSS_PLUGIN_URI . 'assets/css/wpfss-style.css'), WPFSS_PLUGIN_VERSION);
			...
			wp_enqueue_script('wpfss-script', WPFSS_PLUGIN_URI . '/assets/js/wpfss-script.js', array('jquery'), WPFSS_PLUGIN_VERSION, true);
		}
